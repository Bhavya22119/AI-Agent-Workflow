...
export default function SettingsPanel({ 
  ...
  return (
    <div className="absolute top-4 right-4 bottom-4 w-80 bg-zinc-950 border border-zinc-800 shadow-xl rounded-xl flex flex-col z-20 animate-in fade-in slide-in-from-right-8">
      <div className="flex items-center justify-between p-4 border-b border-zinc-800">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onClose} className="p-1.5 text-zinc-400 hover:text-zinc-600 hover:bg-zinc-800 rounded-md transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        <div className="space-y-2">
          <label className="text-sm font-medium text-zinc-300">Node Label</label>
          <Input 
            value={data.label as string} 
            onChange={(e) => handleLabelChange(e.target.value)}
            className="h-9 bg-zinc-900 border-zinc-800 text-white"
          />
        </div>
        ...
        {data.type === 'webhook_trigger' && (
          <div className="space-y-4">
            <div className="p-3 bg-blue-950/30 border border-blue-900/50 rounded-lg space-y-2">
              <p className="text-xs text-blue-400">Send a POST request to this workflow's webhook URL to trigger it.</p>
              <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded p-1.5">
                <code className="text-[10px] text-zinc-300 break-all overflow-hidden line-clamp-2 select-all">
                  {webhookUrl}
                </code>
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-300">Webhook Secret (Optional)</label>
              <Input 
                type="password"
        ...
        {data.type === 'schedule_trigger' && (
          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-300">Cron Expression</label>
            <Input 
              value={config.cron_expression || ''} 
              onChange={(e) => handleConfigChange('cron_expression', e.target.value)}
              placeholder="*/5 * * * *"
            />
            <p className="text-xs text-zinc-400">Standard cron format (e.g. every 5 mins)</p>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea 
                ...
                rows={5}
              />
              <p className="text-xs text-zinc-400">You can use {'{{node_1.output.key}}'} syntax to inject data from previous nodes.</p>
            </div>
          </div>
        )}
        ...
        {data.type === 'conditional_branch' && (
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-300">Condition (Expression)</label>
              <Input 
        ...
        {data.type === 'db_write' && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-300">Payload (JSON)</label>
              <Textarea 
                value={config.payload || ''} 
                onChange={(e) => handleConfigChange('payload', e.target.value)}
                placeholder='{"name": "{{input.name}}"}'
                className="font-mono text-xs bg-zinc-900 border-zinc-800 text-white placeholder:text-zinc-600"
                rows={5}
              />
            </div>
          </div>
