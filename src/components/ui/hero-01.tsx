...
import {
  ...
  ReactIcon,
  TailwindCSSIcon,
} from "@/components/ui/hero-01-utils/hero-01-icons"

export function Hero01() {
  return (
    <div className="relative w-screen overflow-hidden py-8">
      <div className="container mx-auto max-sm:px-2">
        <div className="screen-line-top screen-line-bottom border-x border-zinc-200 md:hidden relative">
          <svg
            className="pointer-events-none absolute inset-0 overflow-visible text-zinc-200"
            viewBox="0 0 210 340"
            fill="none"
        ...
        <div className="screen-line-top screen-line-bottom hidden border-x border-zinc-200 md:block relative">
          <svg
            className="pointer-events-none absolute inset-0 overflow-visible text-zinc-200"
            viewBox="0 0 340 210"
            fill="none"
...
function MainContent({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "flex flex-col justify-center overflow-hidden p-4 lg:p-8 relative z-10",
        className
      )}
    >
      <h1 className="mb-4 font-heading text-[2.5rem]/none font-medium tracking-tight text-zinc-900 sm:mb-6 sm:text-6xl md:text-5xl lg:text-6xl xl:text-7xl">
        Build. Automate. Scale.
      </h1>

      <p className="mb-6 text-base leading-normal text-zinc-600 sm:mb-8 sm:text-xl sm:text-balance md:text-lg lg:text-xl">
        AgentFlow{" "}
        <button
          className="relative top-0.75 inline-flex transition-[scale] outline-none active:scale-[0.97] sm:top-1"
          aria-label="Pronunciation"
        >
          <Volume2Icon className="size-[1em] text-zinc-400" />
        </button>{" "}
        provides{" "}
        <strong className="font-semibold text-zinc-900">enterprise-grade,</strong>{" "}
        <strong className="font-semibold text-zinc-900">secure</strong>{" "}
        workflow automation with humans in the loop.
      </p>

      <div className="mb-6 grid grid-cols-2 items-center gap-4 sm:mb-8 sm:flex justify-center">
        <Button className="px-4 sm:px-8 bg-zinc-900 hover:bg-zinc-800 text-white border-0 rounded-full h-11" onClick={() => window.location.href='/signup'}>
          Sign Up
        </Button>
        <Button className="px-4 sm:px-8 border border-zinc-200 hover:bg-zinc-100 text-zinc-700 bg-white rounded-full h-11" variant="ghost" onClick={() => window.location.href='/login'}>
          Log In
        </Button>
      </div>
...
function TechItem({ icon, title }: { icon: JSX.Element; title: string }) {
  return (
    <div className="flex items-center space-x-2 text-zinc-500 select-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:w-6 [&_svg:not([class*='size-'])]:h-6">
      {icon}
