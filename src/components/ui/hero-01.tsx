  ...
  return (
    <div className="relative w-screen overflow-hidden py-8">
      <div className="container mx-auto max-sm:px-2">
        <div className="screen-line-top screen-line-bottom border-x border-slate-800 md:hidden relative">
          <svg
            className="pointer-events-none absolute inset-0 overflow-visible text-slate-800"
            viewBox="0 0 210 340"
            fill="none"
        ...
        <div className="screen-line-top screen-line-bottom hidden border-x border-slate-800 md:block relative">
          <svg
            className="pointer-events-none absolute inset-0 overflow-visible text-slate-800"
            viewBox="0 0 340 210"
            fill="none"
...
function MainContent({ className }: { className?: string }) {
  return (
    ...
    >
      <h1 className="mb-4 font-heading text-[2.5rem]/none font-medium tracking-tight text-white sm:mb-6 sm:text-6xl md:text-5xl lg:text-6xl xl:text-7xl">
        Build. Automate. Scale.
      </h1>

      <p className="mb-6 text-base leading-normal text-slate-400 sm:mb-8 sm:text-xl sm:text-balance md:text-lg lg:text-xl">
        AgentFlow{" "}
        <button
        ...
        provides{" "}
        <strong className="font-normal text-white">enterprise-grade,</strong>{" "}
        <strong className="font-normal text-white">secure</strong>{" "}
        workflow automation with humans in the loop.
      </p>

      <div className="mb-6 grid grid-cols-2 items-center gap-4 sm:mb-8 sm:flex">
        <Button className="px-4 sm:px-8 bg-indigo-500 hover:bg-indigo-600 text-white border-0" onClick={() => window.location.href='/onboarding'}>
          Get Started Free
        </Button>

        <Button className="px-4 sm:px-8 border border-slate-700 hover:bg-slate-800 text-slate-300" variant="ghost" onClick={() => window.location.href='/login'}>
          Log In
        </Button>
      </div>
...
function TechItem({ icon, title }: { icon: JSX.Element; title: string }) {
  return (
    <div className="flex items-center space-x-2 text-slate-400 select-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:w-6 [&_svg:not([class*='size-'])]:h-6">
      {icon}
