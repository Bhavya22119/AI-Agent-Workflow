...
export default function WorkflowRunPage() {
  ...
  const getStepColor = (status: string) => {
    switch (status) {
      ...
      case 'skipped': return 'bg-zinc-200/50 text-zinc-400 border-zinc-200/30';
      default: return 'bg-zinc-100 text-zinc-400 border-zinc-200';
    }
  };

  if (loading && !run) return <div className="p-8 text-center text-zinc-400">Loading run details...</div>;
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <div className="flex items-center gap-3">
            <p className="text-zinc-400 text-sm font-mono">{runId.slice(0, 8)}...</p>
            {isLive && (
      ...
      <div className="space-y-3 mt-8">
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={step.id}>
              <Card className={`bg-zinc-950 border-zinc-800 transition-all duration-500 ${
                step.status === 'running' ? 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)] bg-zinc-900' : 
                step.status === 'paused' ? 'border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.3)] bg-zinc-900' : ''
              }`}>
                ...
                {step.status === 'completed' && step.output && (
                  <div className="mt-4 p-3 bg-zinc-900 rounded border border-zinc-800 overflow-x-auto">
                ...
                {step.status === 'failed' && step.error && (
                  <div className="mt-4 p-3 bg-zinc-900 rounded border border-rose-900/50 overflow-x-auto">
                ...
                {step.status === 'paused' && step.workflow_step?.type === 'approval_gate' && (
                  <div className="mt-4 p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                    <div className="flex justify-between items-center">
                      ...
                      {role !== 'viewer' && (
                        <button 
                          className="px-4 py-2 rounded-md font-medium text-sm transition-colors bg-amber-500 hover:bg-amber-600 text-white shadow-md disabled:opacity-50"
              ...
              {idx < steps.length - 1 && (
                <div className="ml-[29px] h-3 w-px bg-zinc-800" />
              )}
            </div>
          );
        })}
        ...
        {steps.length === 0 && !loading && (
          <div className="text-center py-8 text-zinc-400 italic">No steps found for this run.</div>
        )}
      </div>
    </div>
  );
